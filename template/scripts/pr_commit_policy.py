...
Generated projects expose only the ``pr`` and ``commits`` subcommands for CI.
The meta-repo template also ships a ``draft`` command for local automation.
"""
...
# ---------------------------------------------------------------------------
# Git subprocess (monkeypatch ``_git_run`` in tests)
# ---------------------------------------------------------------------------
...
def _git_run(
    ...
    *,
    ...
    """Run ``git`` with argument vector ``argv`` (everything after the ``git`` executable)."""
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry: dispatch ``pr`` or ``commits`` subcommands for CI."""
    ...
    if args.command == "commits":
        if not args.base or not args.head:
            logger.error("commits requires --base and --head (or PR_BASE_SHA and PR_HEAD_SHA)")
            return 1
        return _cmd_commits(args.base, args.head)
    return 1  # pragma: no cover -- argparse required subcommand makes this unreachable
