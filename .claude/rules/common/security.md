...
This is a Copier template repository. Security considerations include:

- The `_tasks` in `copier.yml` execute arbitrary shell commands on the user's machine.
  Keep tasks minimal, idempotent, and auditable.
- Generated projects inherit security configuration from the template; changes to
  template security configuration affect all future generated projects.
- Copier's `--trust` flag must be documented so users understand what they are consenting to.
