...
`CLAUDE.md` (root and `template/CLAUDE.md.jinja`) is the primary context document
for AI assistants. Keep it up to date:

- Run `/update-claude-md` slash command to detect drift against `pyproject.toml`
  and the `justfile`.
- Update `CLAUDE.md` when you add or change slash commands, hooks, tooling, or
  project structure.
- Do not duplicate content between `CLAUDE.md` and the rules files. Cross-reference
  with a link instead.

## Generated project documentation

Documentation for generated projects lives in `docs/` (MkDocs) and follows the same
conventions. The `docs/index.md.jinja` template is the starting point.
